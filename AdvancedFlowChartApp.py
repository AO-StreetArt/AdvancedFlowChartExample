...
class ConnectorButton(Image):
    ...
    def on_touch_down(self, touch):
        ...
        return super(ConnectorButton, self).on_touch_down(touch)
...
class ConnectorNode(BoxLayout):
    ...
    def double_press(self, touch, *args):
        self.double_press = touch.pos
        Logger.debug('Node pressed at %s' % (self.double_press))
...
#This class will help to track links
class FC_Link(Widget):
    start_cell=ObjectProperty(None)
    end_cell=ObjectProperty(None)
        
class FlowchartExampleWidget(GridLayout):
    drag_grid=ObjectProperty(None)
    flow_nodes=ListProperty([])
    
    #We can track links with this property & the FC_Link class
    flow_links=ListProperty([])
...
class FlowchartExampleApp(App):
    def build(self):
        ...
        Clock.schedule_interval(self.double_press, 3)
        return root
        
    def double_press(self, *args):
        for node in self.root.flow_nodes:
            for option in node.connected_nodes:
                if option.is_set:
                    if option.has_button==False:
                        #Add a button on top of the draggable node
                        drag = ConnectorNode(app=self, grid=self.root.drag_grid, cell=option.cell)
                        drag.bind(on_double_press=self.double_press)
                        self.root.flow_nodes.append(drag)
                        option.cell.add_widget(drag)
                        option.has_button=True
                        Logger.debug('Add a button to double clicked node')
                else:
                    if option.has_button:
                        #Remove the button and the rest of the chain
                        Logger.debug('Remove chain')
